...
def df_preprocess(path):
    ...
    price_df.loc[:, 'cash_close'] = 1

    onehot_encode = OneHotEncoder()
    onehot_encode.fit(price_df.columns.values.reshape(-1, 1))
    ret_df = np.log(price_df).diff()
    max_col = ret_df.apply(lambda x: x.idxmax(), axis=1).values[1:]
    best_ratio = onehot_encode.transform(max_col.reshape(-1, 1)).toarray()
    dataframe = dataframe.iloc[:-1, ]
    return dataframe, best_ratio, price_columns


df, best_ratio, price_columns = df_preprocess('./data/create_feature.csv')
